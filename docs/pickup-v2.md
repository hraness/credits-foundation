...
## Returning top-up wire support

`parseCreditsTopupCreateV2` and `parseCreditsTopupCreatedV2` admit the separate `hraness-credits-topup-create-v2` and `hraness-credits-topup-created-v2` schema literals. Their creation tuple/bounds match registration, but neither accepts the other mode's schema. The existing device bearer belongs in Authorization, never the body. The creation reply contains no credential, current balance, catalogue or payment state. Optional pack is an initial checkout preference, not a fixed final charge.

`parseCreditsTopupStatusV2` validates the payment-only subset of the existing `hraness-credits-claim-status-v1` wire from device-authenticated `/v1/claims/{claimId}`. Its expectation binds claim ID, original createdAt and exact expiresAt. It admits pending/paid/expired, rejects consumed and every token/pickup field, requires paidAt only for paid, and permits late payment after unpaid expiry. Optional money must have consistent safe integer micro-USD/credits/USD projections within the existing v1 money bound. It does not infer a grant amount from a changing wallet balance.

Pure creation keeps the authority’s 128-character claim bound; the status parser and recovery-state admission retain the actual follow-on route's 64-character bound. A longer created ID cannot expose a usable payment URL through this state model. Invalid responses preserve the saved intent. Neither 401 nor a malformed/error response proves revocation or expiry.

`tests/fixtures/topup-v2-authority.ts` retains nine nonsecret raw actual-handler responses from authority commit 6b1b943: creation, exact/expired/paid replay, pending/expired/paid status and conflict/unauthorized errors. Its source hashes describe the original capture. The subsequent filename-only `topups-v2.ts` to `topupsV2.ts` repair retains exact module bytes; only internal reference names changed, with no wire/activation changes. Tests have no sibling dependency or live request. Transport and the authority route remain inactive.
